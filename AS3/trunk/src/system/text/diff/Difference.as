...
package system.text.diff 
{
	...
	public class Difference 
	{
		...
		public static function cleanupEfficiency( diffs:Array ):void 
		{
  			...
  			var equalitiesLength:int  = 0     ; // Keeping our own length var is faster in JS.
  			
  			var lastequality:String   = ''    ; // Always equal to equalities[equalitiesLength-1][1]
  			
  			var pointer:int           = 0     ; // Index of current position.
		...
		public static function cleanupSemantic( diffs:Array ):void
		{
  			...
  			var pointer:int          = 0     ; // Index of current position.
  			var length_changes1:int  = 0     ; // Number of characters that changed prior to the equality.
  			var length_changes2:int  = 0     ; // Number of characters that changed after the equality.
  			while (pointer < diffs.length) 
  			{
    			if ( diffs[pointer][0] == EQUAL ) // equality found 
    			{  
			...
			// Intentionally ignore the first and last element (don't need checking).
  			while (pointer < diffs.length - 1) 
  			{
    			if (diffs[pointer - 1][0] == EQUAL && diffs[pointer + 1][0] == EQUAL) 
    			{
      				...
      				var commonOffset:int = commonSuffix(equality1, edit) ;
      				if (commonOffset) 
      				{
      				...
      				var bestScore:int       = cleanupSemanticScore(equality1, edit, equality2) ;
      				...
      				while (edit.charAt(0) === equality2.charAt(0)) 
      				{
        				...
        				var score:int = cleanupSemanticScore(equality1, edit, equality2);
        				if (score >= bestScore) 
        				{
	          				bestScore     = score ;
		...
		public static function commonPrefix( text1:String , text2:String ):uint 
		{
  			...
  			var pointermax:int   = Math.min(text1.length, text2.length);
  			var pointermid:int   = pointermax;
	 		var pointerstart:int = 0 ;
		 ...
		 */
		public static function fromDelta( text:String, delta:String ):Array 
		{
  			var diffs:Array      = [] ;
  			var diffsLength:int = 0  ;  // Keeping our own length var is faster in JS.
  			var pointer:int     = 0  ;  // Cursor in text
  			...
  			for (var x:uint = 0 ; x < len ; x++ ) 
  			{
    			...
    			switch (tokens[x].charAt(0)) 
    			{
    				case '+' :
    				{
      					...
      					{
        					throw new Error('Illegal escape in fromDelta: ' + param) ; // Malformed URI sequence.
      					}
      					break ;
    				}
    				case '-' :
    				...
    				case '=' :
    				{
     					...
     					var txt:String = text.substring(pointer, pointer += n);
     					if( (tokens[x].charAt(0) == '=') )
     					{
       					    diffs[diffsLength++] = [EQUAL, txt] ;
     					}
     					else
     					{
     					    diffs[diffsLength++] = [DELETE, txt] ;
     					} 
						break;
    				}
    				default :
    				{
      					if ( tokens[x] ) 
      					{
        					throw new Error('Invalid diff operation in fromDelta : ' + tokens[x]) ; // Blank tokens are ok (from a trailing \t). Anything else is an error.
      					}
    				}
    			}
  			}
  			...
  			{
    			throw new Error('Delta length (' + pointer + ') does not equal source text length (' + text.length + ').') ;
  			}
  			return diffs ;
		}		
		...
		public static function linesToChars( text1:String, text2:String ):Array 
		{
            ...
            function linesToCharsMunge( text:String ):String 
            {
                ...
                var lineEnd   :Number  = -1 ;
            
                // Keeping our own length variable is faster than looking it up.
                
                var size:uint = lineArray.length;
                ...
                while ( lineEnd < (text.length - 1) ) 
                {
                    ...
                    {
                        chars               += String.fromCharCode( size ) ;
                        lineHash[line]       = size ;
                        lineArray[ size++ ]  = line ;
                    }
                }
  			...
  			var chars1:String   = linesToCharsMunge( text1 ) ;
  			var chars2:String   = linesToCharsMunge( text2 ) ;
		...
		public static function main( text1:String, text2:String, checklines:Boolean=true ):Array 
		{
			...
			// Trim off common prefix (speedup)
  			...
  			var commonlength:int    = commonPrefix( text1, text2 ) ;
		...
		public static function map( text1:String, text2:String ):Array 
		{
	  		...
	  		var front:int = (text1.length + text2.length) % 2;
   				...
   				if ( doubleEnd ) 
   				{
      				...
      				for (k = -d; k <= d; k += 2) 
      				{
        				...
        				if (done) 
        				{
          					...
          					ar1 = ar1.slice(0, footsteps[footstep] + 1) ;
          					var a:Array = path1(ar1, text1.substring(0, text1.length - x), text2.substring(0, text2.length - y));
          					return a.concat( path2(ar2, text1.substring(text1.length - x), text2.substring(text2.length - y))) ;
        				}
		...
		public static function prettyHtml( diffs:Array ):String 
		{
  			...
  			for (var x:uint = 0; x < len ; x++ ) 
  			{
    			var m:int    = diffs[x][0]; // Mode (delete, equal, insert)
    			var t:String = diffs[x][1]; // Text of change.
    			var i:int    = diffs[x][2]; // Index of change.
		...
		public static function xIndex(diffs:Array, loc:Number):Number 
		{
  			...
  			{
    			return last2 ; // Was the location was deleted ?
  			}
  			return last2 + (loc - last1) ; // Add the remaining character length.
		}
		...
		private static function halfMatchI( longtext:String , shorttext:String , i:Number ):Array 
		{
    		...
    		while ((j = shorttext.indexOf(seed, j + 1)) != -1) 
    		{
      			...
      			var prefixLength:int = commonPrefix(longtext.substring(i)    , shorttext.substring(j) ) ;
      			
      			var suffixLength:int = commonSuffix(longtext.substring(0, i) , shorttext.substring(0, j) ) ;
		 ...
		 * @param {Array.<Object>} ar Array of paths.
		 * @param text1 Old string fragment to be diffed.
		 * @param text2 New string fragment to be diffed.
		 * @return Array of diff tuples {Array.<Array.<*>>}.
 		 */
		private static function path1( ar:Array , text1:String, text2:String):Array 
		{
  			var last:int    ;
  			var path:Array  = [];
		 ...
		 * @param ar Array of paths. {Array.<Object>} 
		 * @param text1 Old string fragment to be diffed
		 * @param text2 New string fragment to be diffed
		 * @return Array of diff tuples {Array.<Array.<*>>} 
		 */
