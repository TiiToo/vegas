...
package lunas.components.buttons 
{
    ...
    public class LabelButtonBuilder extends BackgroundButtonBuilder 
    {
        ...
        public override function update():void 
        {
            ...
            if ( b  && s )
            {
                field.x      = EdgeMetrics.filterNaNValue( s.padding.left ) + EdgeMetrics.filterNaNValue( s.margin.left ) ;
                field.y      = EdgeMetrics.filterNaNValue( s.padding.top )  + EdgeMetrics.filterNaNValue( s.margin.top  ) ;
                if ( s.autoSize )
                {
                    refreshField()  ;
                    field.width  = field.textWidth ;
                    field.height = field.textHeight ;
                    b.hack::_w = field.width  + EdgeMetrics.filterNaNValue( s.padding.horizontal ) + EdgeMetrics.filterNaNValue( s.margin.horizontal ) ;
                    b.hack::_h = field.height + EdgeMetrics.filterNaNValue( s.padding.vertical   ) + EdgeMetrics.filterNaNValue( s.margin.vertical   ) ;
                    refreshBackground() ;
                }
                else
                {
        ...
        protected function refreshField():void
        {
            ...
            if ( b  && s )
            {
                ...
                field.width            = b.w - EdgeMetrics.filterNaNValue( s.padding.horizontal ) - EdgeMetrics.filterNaNValue( s.margin.horizontal ) ;
                field.height           = b.h - EdgeMetrics.filterNaNValue( s.padding.vertical   ) - EdgeMetrics.filterNaNValue( s.margin.vertical   ) ;
                field.autoSize         = s.autoSize ? TextFieldAutoSize.LEFT : TextFieldAutoSize.NONE ;
                field.filters          = s.labelFilters ;
