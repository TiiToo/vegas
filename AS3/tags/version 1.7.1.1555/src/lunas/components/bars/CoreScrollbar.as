...
package lunas.components.bars
{
     ...
     */
    public class CoreScrollbar extends CoreProgressbar implements Scrollbar
    {
        /**
         ...
         */
        public function CoreScrollbar( direction:String = "vertical", w:Number = 150 , h:Number = 6, id:* = null, isFull:Boolean=false, name:String = null )
        {
            super( direction, w, h, id, isFull, name ) ;
        }
        ...
        public function get thumbSize():Number
        {
            ...
            if( isNaN(_thumbSize) )
            {
                return ( direction == Direction.HORIZONTAL ) ? ( h - vertical ) : ( w - horizontal ) ;
            }
            else
            {
        ...
        public override function viewPositionChanged( flag:Boolean=false ):void 
        {
            try
            {
                ...
                if ( !isDragging )
                {
                    ...
                    {
                        range = bar.width - ts ;
                        thumb.x = _area.x + (_position - _min) / (_max - _min) * range ;
                        thumb.y = _area.y ;
                        if ( _invert )
                        {
                            thumb.x = _area.x + range - thumb.x ;
                        }
                    }
                    else
                    {
                        range = bar.height - ts ;
                        thumb.x = _area.x ;
                        thumb.y = _area.y + (_position - _min) / (_max - _min) * range ;
                        if ( _invert )
                        {
                            thumb.y = _area.y + range - thumb.y ;
                        }
                    }
                }
            }
