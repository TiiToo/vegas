...
package pegas.colors 
{
    ...
    public class RGBA extends RGB 
    {
         ...
         */
        public static function numberToRGBA( value:Number ):RGBA
        {
            var a:int, r:int, g:int, b:int, gb:int ;
            a  = value >> 24 ;
            r  = value >> 16 ;
            gb = value ^ r << 16 ;
            g  = gb >> 8 ;
         ...
         */
        public override function interpolate( c:RGB , level:Number = 1 ):RGB
        {
            var p:Number = Range.UNITY_RANGE.clamp( isNaN(level) ? 1 : level ) ;
            var q:Number = 1 - p ;
            return new RGBA
            (
                _alpha * q + (c as RGBA).alpha * p  ,
                _red   * q + c.red   * p ,
                _green * q + c.green * p ,
                _blue  * q + c.blue  * p  
            ) ;
